...
export default function DoctorLogin() {
  ...
  const { login } = useAuth();
  const [email, setEmail] = useState("doctor1@gov.in");
  ...
  async function submit(e) {
    ...
    try {
      ...
      // role check happens in route guard
      nav("/doctor/token-search");
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-sky-200 via-indigo-200 to-pink-200">
      <div className="w-[380px] bg-white/40 backdrop-blur-md border border-white/50 rounded-3xl p-6 shadow-xl">
        <div className="text-center">
          <div className="text-2xl tracking-widest text-slate-800">User Login</div>
          <div className="text-xs text-slate-500 mt-2">Doctor Portal</div>
        </div>
